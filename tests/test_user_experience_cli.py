...
class UserExperienceCliTests(unittest.TestCase):
    def test_release_version_and_roadmap_cli_are_informational_only(self) -> None:
        ...
        self.assertEqual(version_output.strip().splitlines(), ["Atlas OS v0.9.1", "Executive Workflow"])
        self.assertIn("Atlas OS Roadmap", roadmap_output)
        self.assertIn("Current Release", roadmap_output)
        self.assertIn("v0.9.1 — Executive Workflow", roadmap_output)
    ...
    def test_release_version_source_is_available_and_synchronized(self) -> None:
        ...
        self.assertEqual(load_version(), "0.9.1")
    ...
    def test_agent_and_inbox_cli(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict("os.environ", env, clear=False):
                ...
                self.assertIn("created_date", inbox)
                self.assertIn("created_time", inbox)
                self.assertIn("updated_date", inbox)
                self.assertIn("updated_time", inbox)
                ...
                self.assertIn("updated_date:", item)
                self.assertIn("updated_time:", item)
