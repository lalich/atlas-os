...
class CommandCenterTests(unittest.TestCase):
    ...
    def test_greenrock_market_pulse_route_returns_200(self) -> None:
        ...
        self.assertIn("market-pulse-card-grid compact-stage-grid", response.body)
        self.assertIn("stageable-card market-pulse-card", response.body)
        self.assertIn("stageable-action", response.body)
        self.assertIn("Score</b>", response.body)
        self.assertIn("Conf.</b>", response.body)
        self.assertIn("Evid.</b>", response.body)
        self.assertNotIn("market-pulse-table compact-source-table", response.body)

    def test_greenrock_staging_uses_bounded_table_layout(self) -> None:
        with _isolated_env() as root:
            run_population_scan(root / "output", "all", provider=FullHistoryProvider())
            dispatch_request("POST", "/greenrock/score/save", "ticker=LC01&list_key=watchlist")
        ...
        self.assertIn("watchlist-stage-cards", response.body)
        self.assertIn("scan-stage-cards", response.body)
        self.assertIn("stageable-card staging-source-card watchlist-stage-card", response.body)
        self.assertIn("stageable-card staging-source-card scan-stage-card", response.body)
        self.assertIn("staging-source-card", response.body)
        self.assertIn("source-metric-chips", response.body)
        self.assertIn("source-notes-row", response.body)
        self.assertIn("source-action-row", response.body)
        self.assertNotIn("watchlist-stage-table", response.body)
        self.assertNotIn("scan-stage-table", response.body)
        self.assertNotIn("watchlist-stage-table picks-table", response.body)
        self.assertNotIn("scan-stage-table picks-table", response.body)
