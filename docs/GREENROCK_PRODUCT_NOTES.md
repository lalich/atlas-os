...
- Keep staging enrichment preview-only and local-only. `atlas greenrock staging enrich` preserves the staged-candidates-only default, while `atlas greenrock staging enrich --scope visible` and the browser **Refresh / Enrich Staging Page** action refresh staged candidates, visible watchlist candidates, and visible latest scan candidates. Staged candidates update the local staging CSV; watchlist/latest scan display values use `.atlas/output/greenrock/staging/enrichment_cache.csv` when source mutation is not appropriate. These actions must not create workflow runs, reports, approvals, PDFs, emails, Slack messages, publications, trades, or client-facing artifacts.
...
- Treat Morning Brief snapshots as local operating-log records under `.atlas/output/atlas/morning_briefs/`. Snapshot save may write JSON only; it must not create reports, approvals, PDFs, emails, publications, trading actions, client files, or external LLM/API calls.
- Keep `atlas morning-brief` and `/atlas/morning-brief` read-only except for explicit snapshot saves. They may point the operator to local next steps, but must not create reports, approvals, PDFs, emails, publications, trading actions, client files, or external LLM/API calls.
...
- Treat Derivative Workbench as a separate research-only options layer. It may analyze staged or manual tickers, store local derivatives snapshots, calculate Derivative Timing Score, rank bounded single-leg calls/puts, and show scenario profiles. It must not mutate GreenRock Score, Confidence, Evidence Agreement, ranking, staging, reports, approvals, PDFs, universes, watchlists, email, Slack, publishing, trading, broker/API orders, client files, credentials, or external LLM/API services.
- Use American-style binomial tree pricing as the Phase 10A source of truth for listed U.S. equity options. Black-Scholes is not the primary model. Barone-Adesi-Whaley may be added later as a comparison/fast approximation only after clean tests.
- Keep Wall options content compact. The Wall should show **Options Manifesto** summary only; detailed chain quality, Greeks, contract rankings, and scenario tables belong in `/greenrock/derivatives`.
